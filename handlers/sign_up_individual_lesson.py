...
@router.callback_query(F.data == 'buy_individual_lesson')
async def buy_individual_lesson(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    coaches = await db_requests.get_coaches()
    if coaches:
        message_text = 'Выберите преподавателя индивидуального занятия.\n\n'
    else:
        message_text = 'К сожалению, на текущий момент нет доступных преподавателей.'

    builder = InlineKeyboardBuilder()
    for coach in coaches:
        builder.button(text=f'{coach.first_name} {coach.last_name}', callback_data=f'select_coach_for_individual_sign_up_{coach.id}')
    builder.button(text='🔙 Личный кабинет', callback_data='profile')    
    builder.adjust(1)
    await callback.message.edit_text(message_text, reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith('select_coach_for_individual_sign_up_'))
async def select_coach_for_individual_sign_up(callback: CallbackQuery, state: FSMContext):
    coach = await db_requests.get_coach_by_id(callback.data[36:])
    ...
    message_text = (f'Выберите месяц.\n\n'
                    f'ℹ️ {coach.first_name + " " + coach.last_name} ➔ индивидуальное занятие')
    available_trainings = await db_requests.get_individual_trainings(coach.id, 
    ...
    for available_month in available_months:
        ...
        builder.button(text=f'{month_names[available_month - 1]}', callback_data=f'select_month_for_individual_sign_up_{available_month}.{year}')
    builder.button(text='🔙 Выбор тренера', callback_data=f'buy_individual_lesson') 
    builder.adjust(1)
    await callback.message.edit_text(message_text, reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith('select_month_for_individual_sign_up_'))
async def select_month_for_individual_sign_up(callback: CallbackQuery, state: FSMContext):
    selected_date = callback.data[36:].split('.')
    ...
    message_text = (f'Выберите день.\n\n'
                    f'ℹ️ {coach.first_name + " " + coach.last_name} ➔ индивидуальное занятие ➔ xx.{selected_month}.{selected_year}')
    ...
    for available_day in sorted_available_days:
        builder.button(text=f'{available_day.day}.{available_day.month}.{available_day.year} ({days[datetime.datetime(available_day.year, available_day.month, available_day.day).weekday()]})', callback_data=f'select_day_for_individual_sign_up_{available_day.day}')
    builder.button(text='🔙 Выбор месяца', callback_data=f'select_coach_for_individual_sign_up_{coach.id}') 
    builder.adjust(1)
    
    await callback.message.edit_text(message_text, reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith('select_day_for_individual_sign_up_'))
async def select_day_for_individual_sign_up(callback: CallbackQuery, state: FSMContext):
    selected_day = callback.data[34:]
    ...
    message_text = (f'Выберите время.\n\n'
                    f'ℹ️ {coach.first_name + " " + coach.last_name} ➔ индивидуальное занятие ➔ {selected_day}.{selected_month}.{selected_year}')
    ...
    for available_training in available_trainings:
        ...
        builder.button(text=f'{"✅ " if user_enrollment else ""}{datetime.datetime.strftime(available_training.training_date, "%H:%M")}', callback_data=f'select_individual_training_for_sign_up_{available_training.id}')
    builder.button(text='🔙 Выбор дня', callback_data=f'select_month_for_individual_sign_up_{selected_month}.{selected_year}') 
    builder.adjust(1)
    await callback.message.edit_text(message_text, reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith('select_individual_training_for_sign_up_'))
async def select_day_for_sign_up_(callback: CallbackQuery, state: FSMContext):
    training_id = callback.data[39:]
    ...
    selected_year = fsmdata.get('selected_year')
    coach: models.Trainer = fsmdata.get('coach')
    ...
    if user_enrollment:
        ...
        builder.adjust(1)
        message_text = (f'Вы забронировали индивидуальное занятие у тренера {coach.first_name + " " + coach.last_name} на {datetime.datetime.strftime(training.training_date, "%d.%m.%Y %H:%M")}.\n\n'
        ...
        for i in range(180):
            ...
            if payment_status.status == 'succeeded':
                ...
                if coach.notification_of_entries == True: 
                    try:
                        await bot.send_message(coach.user_telegram_id, f'Ученик {user.first_name + " " + user.last_name} ({user.phone_number}) оплатил индивидуальное занятие на {datetime.datetime.strftime(training.training_date, "%d.%m.%Y %H:%M")}')
                    except:
                        pass
                return
        ...
        try:
            ...
            pass
        await callback.message.answer(f'🤷‍♂️ Время на оплату индивидуального занятия у тренера {coach.first_name + " " + coach.last_name} в {datetime.datetime.strftime(training.training_date, "%d.%m.%Y %H:%M")} вышло или платёж завершился с ошибкой.\n\nБронирование отменено.')
