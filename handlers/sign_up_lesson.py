...
async def sign_up_lesson(callback: CallbackQuery, state: FSMContext):
    ...
    actual_subscription = await db_requests.get_actual_subscription(callback.from_user.id)
    user_subscriptions = await db_requests.get_all_actual_user_subscriptions(callback.from_user.id)
    if not user_subscriptions and not actual_subscription:
        await callback.answer('Вы не можете записаться на занятие, т.к. у вас нет действующего абонемента.', show_alert=True)
        return
    if coaches:
        message_text = ('❗️<b>ВНИМАНИЕ!</b> Отмена записи возможна не позже, чем за 15 часов до начала занятия!\n\n'
                        'Выберите преподавателя группового занятия.\n\n')
    else:
...
async def select_direction_for_sign_up_(callback: CallbackQuery, state: FSMContext):
    ...
    for available_month in available_months:
        ...
        builder.button(text=f'{month_names[available_month - 1]}', callback_data=f'select_month_for_sign_up_{available_month}.{year}')
    ...
    if user_enrollment:
        ...
        for user_subscription in user_subscriptions:
            ...
            if user_subscription.purchase_date <= training_date <= user_subscription.expiration_date:
                user_enrollments_in_period = await db_requests.get_user_enrollments(user_telegram_id=callback.from_user.id, date_from=user_subscription.purchase_date, date_to=user_subscription.expiration_date, without_individual=True)
                ...
                '''if coach.notification_of_entries == True: 
                    try:
                        await bot.send_message(coach.user_telegram_id, f'Ученик {user.first_name + " " + user.last_name} записался на занятие {training.direction_of_training} на {datetime.datetime.strftime(training.training_date, "%d.%m.%Y %H:%M")}')
                    except:
