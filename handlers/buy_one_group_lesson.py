...
@router.callback_query(F.data == 'buy_one_group_lesson')
async def trial_lesson(callback: CallbackQuery, state: FSMContext):
    ...
    for coach in coaches:
        builder.button(text=f'{coach.first_name} {coach.last_name}', callback_data=f'select_coach_for_one_group_lesson_sign_up_{coach.id}')
    builder.button(text='🔙 Личный кабинет', callback_data='profile')    
    builder.adjust(1)
    await callback.message.edit_text(message_text, reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith('select_coach_for_one_group_lesson_sign_up_'))
async def select_coach_for_trial_sign_up(callback: CallbackQuery, state: FSMContext):
    coach = await db_requests.get_coach_by_id(callback.data[42:])
    ...
    for direction in coach.directions_of_teaching:
        if (direction.name in ['Индивидуальное', 'Индивидуальное парное'] or 
            ...
            continue
        try:
            builder.button(text=f'{direction.name.split("(")[0]}', callback_data=f'select_direction_for_one_group_session_sign_up_{direction.id}')     
        except:
            builder.button(text=f'{direction.name}', callback_data=f'select_direction_for_one_group_session_sign_up_{direction.id}')
    builder.button(text='🔙 Выбор тренера', callback_data='buy_one_group_lesson') 
    builder.adjust(1)
    await callback.message.edit_text(message_text, reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith('select_direction_for_one_group_session_sign_up_'))
async def select_direction_for_trial_sign_up(callback: CallbackQuery, state: FSMContext):
    direction = await db_requests.get_direction_by_id(callback.data[47:])
    ...
    for available_month in available_months:
        ...
        builder.button(text=f'{month_names[available_month - 1]}', callback_data=f'select_month_for_one_group_session_sign_up_{available_month}.{year}')
    builder.button(text='🔙 Выбор направления', callback_data=f'select_coach_for_one_group_lesson_sign_up_{coach.id}') 
    builder.adjust(1)
    await callback.message.edit_text(message_text, reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith('select_month_for_one_group_session_sign_up_'))
async def select_month_for_trial_sign_up(callback: CallbackQuery, state: FSMContext):
    selected_date = callback.data[43:].split('.')
    ...
    for available_day in sorted_available_days:
        builder.button(text=f'{available_day.day}.{available_day.month}.{available_day.year} ({days[datetime.datetime(available_day.year, available_day.month, available_day.day).weekday()]})', callback_data=f'select_day_for_one_group_session_sign_up_{available_day.day}')
    builder.button(text='🔙 Выбор месяца', callback_data=f'select_direction_for_one_group_session_sign_up_{selected_direction.id}') 
    builder.adjust(1)
    
    await callback.message.edit_text(message_text, reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith('select_day_for_one_group_session_sign_up_'))
async def select_day_for_one_group_session_sign_up_(callback: CallbackQuery, state: FSMContext):
    selected_day = callback.data[41:]
    ...
    for available_training in available_trainings:
        ...
        builder.button(text=f'{"✅ " if user_enrollment else ""}{datetime.datetime.strftime(available_training.training_date, "%H:%M")}', callback_data=f'select_training_for_one_group_session_sign_up_{available_training.id}')
    builder.button(text='🔙 Выбор дня', callback_data=f'select_month_for_one_group_session_sign_up_{selected_month}.{selected_year}') 
    builder.adjust(1)
    await callback.message.edit_text(message_text, reply_markup=builder.as_markup())


@router.callback_query(F.data.startswith('select_training_for_one_group_session_sign_up_'))
async def select_training_for_trial_sign_up(callback: CallbackQuery, state: FSMContext):
    training_id = callback.data[46:]
    training = await db_requests.get_training_by_id(training_id)
    training_enrollments = await db_requests.get_training_enrollments(training.id)
    fsmdata = await state.get_data()
    selected_direction = fsmdata.get('selected_direction')
    selected_month = fsmdata.get('selected_month')
    ...
    if user_enrollment:
        if (training.training_date - (datetime.datetime.utcnow() + datetime.timedelta(hours=7))).total_seconds() / 3600 < 15:
            ...
            return
        await callback.answer('Запись отменена. Для возврата средств, пожалуйста, обратитесь к сотрудникам школы.', show_alert=True) 
        await db_requests.unenroll_training(callback.from_user.id, training.id)
    else:
        if len(training_enrollments) >= training.maximum_students:
            await callback.answer('К сожалению, на эту тренировку записано максимальное количество учеников.', show_alert=True)
            return
        await db_requests.change_user_probe_status(callback.from_user.id, True)
        await db_requests.enrollment_on_training(callback.from_user.id, training.id, single_group=True)
        payment = Payment.create({
        "amount": {
            "value": f"{other_settings.single_lesson_cost}.00",
            "currency": "RUB"
        },
        "confirmation": {
            "type": "redirect",
        ...
        "capture": True,
        "description": f"Оплата одного группового занятия {datetime.datetime.strftime(training.training_date, '%d.%m.%Y %H:%M')}"
        }, uuid.uuid4())
        ...
        builder.adjust(1)
        message_text = (f'Вы забронировали одно групповое занятие у тренера {coach.first_name + " " + coach.last_name} на {datetime.datetime.strftime(training.training_date, "%d.%m.%Y %H:%M")}.\n\n'
